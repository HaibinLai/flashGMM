 ...
 *      in the hot loop
 *   4. Online log-sum-exp state in shared memory — freed registers for computation
 *
 * Two core kernels implementing Flash-GMM:
 *   Kernel 1 (flash_log_normalizer_v2): Pass 1 — online log-sum-exp → log_normalizer[N]
 *   Kernel 2 (flash_accumulate_stats_v2): Pass 2 — recompute γ, accumulate n_k, s_k, sq_k
 *
 * Together these implement one full EM iteration with ZERO N×K materialization.
 */
...
//
// Moves ALL logf() out of the hot kernels.
// log_coeff[k] = log_pi[k] - 0.5 * (d * LOG_2PI + Σ_j log(var[k,j]))
// ============================================================================
...
// ============================================================================
// Kernel 1 v2: Flash Log-Normalizer (Pass 1 of fused E+M)
//
// Grid:  ceil(N / BN)
// Block: BLOCK_THREADS (multiple of 32)
//
...
//   - All lanes compute online log-sum-exp (identical state)
//
// Logsumexp state persists across centroid tiles via shared memory.
//
// Shared memory layout:
//   mu_tile    [BK * d]       centroid means for current tile
//   iv_tile    [BK * d]       pre-computed 1/var
//   lc_tile    [BK]           pre-computed log_coeff
//   max_smem   [BN]           per-point running max
//   sum_smem   [BN]           per-point running sum_exp
// ============================================================================
...
__global__ void flash_log_normalizer_v2_kernel(
    ...
    for (int t = 0; t < K; t += BK_COMPILE) {
        ...
        for (int pt = warp_id; pt < BN_actual; pt += n_warps) {
            ...
            // Load logsumexp state (lane 0 reads, broadcast to all lanes)
            ...
            for (int kk = 0; kk < tile_k; kk++) {
                // Warp-collaborative Mahalanobis distance
                float partial = 0.0f;
                #pragma unroll
                ...
                // Online log-sum-exp (all lanes compute identical result)
                if (ll > rm) {
    ...
    // Write log_normalizer to global memory
...
// ============================================================================
// Kernel 2 v2: Flash Accumulate Statistics (Pass 2 of fused E+M)
//
// Grid:  (ceil(N/BN), ceil(K/BK))
// Block: BLOCK_THREADS
//
// Each warp:
//   - Handles a subset of points serially
//   - Warp-collaborative distance via lane-striped dimensions
//   - Accumulates to its OWN shared memory section (ZERO atomics in hot loop)
//
// After all points: inter-warp tree reduction, single global atomicAdd.
//
// Shared memory layout:
//   mu_tile    [BK * d]
//   iv_tile    [BK * d]
//   lc_tile    [BK]
//   warp_nk    [n_warps * BK]
//   warp_sk    [n_warps * BK * d]
//   warp_sqk   [n_warps * BK * d]
// ============================================================================
...
__global__ void flash_accumulate_stats_v2_kernel(
    ...
    const int warp_id = tid >> 5;
    const int lane_id = tid & 31;
    const int n_warps = blockDim.x >> 5;  // = 8 for BLOCK_THREADS=256
    ...
    for (int pt = warp_id; pt < BN_actual; pt += n_warps) {
        ...
        // Load X dims (lane-striped, coalesced within warp)
        ...
        for (int kk = 0; kk < tile_k; kk++) {
            ...
            // Accumulate to per-warp storage (lane-striped — NO conflicts)
            if (lane_id == 0)
    ...
    // ---- Warp 0 → global (one atomicAdd per block) ----
    if (warp_id == 0) {
        for (int idx = lane_id; idx < tile_k; idx += 32)
...
// ============================================================================
// Kernel 3: Compute new parameters from sufficient statistics (unchanged)
// ============================================================================
...
// Standard E-step (materializes L — baseline)
...
// Flash E+M fused — Optimized v2 (ZERO N×K materialization)
std::vector<torch::Tensor> flash_em_fused_cuda(
    ...
    // Pre-computed buffers (live only during this EM iteration)
    ...
    const int n_warps = BLOCK_THREADS / 32;  // = 8

    // ---- Auto-select BK for each kernel ----
    // Kernel 1 shared mem: BK1*(2d+1) + 2*BN — small overhead, use large BK
    int BK1 = std::min(32, K);
    // Round to nearest template value
    if (BK1 >= 32) BK1 = 32;
    else if (BK1 >= 16) BK1 = 16;
    else if (BK1 >= 8) BK1 = 8;
    else BK1 = 4;

    // Kernel 2 shared mem: BK2*(2d+1)*(1+n_warps) — budget ~100KB
    int BK2;
    {
        int budget_floats = 100 * 1024 / 4;  // 100KB in floats
    ...
    // ---- Compute new parameters from sufficient statistics ----
    {
